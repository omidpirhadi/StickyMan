						...
						if( optionItems.Length > 0 )
						{
							...
							switch( itemIds[ 0 ] )
							{
								...
								case "Field":
								{
									...
									{
										float result = 0;
										if( float.TryParse( optionItems[ 3 ], out result ) )
											currentOption.DefaultFieldValue = result;
										else
											Debug.LogError( "[AmplifyShaderEditor] Wrong field format. Current: " + optionItems[ 3 ] + " | Expected a valid float number" );
									}
									...
									if( optionItems.Length >= 6 )
									{
										...
										float min = 0;
										if( float.TryParse( optionItems[ 4 ], out min ) )
											currentOption.FieldMin = min;
										else
											Debug.LogError( "[AmplifyShaderEditor] Wrong field format. Current: " + optionItems[ 4 ] + " | Expected a valid float number" );

										float max = 1;
										if( float.TryParse( optionItems[ 5 ], out max ) )
											currentOption.FieldMax = max;
										else
											Debug.LogError( "[AmplifyShaderEditor] Wrong field format. Current: " + optionItems[ 5 ] + " | Expected a valid float number" );
									}
